<template>
        <!-- le persistent sert à rester au premier plan de l'écran -->
    <v-dialog width="350px" persistent v-model="registerDialog">
        <!-- l'activator permet de distinguer si l'on registered ou pas -->
        <v-btn class="error" accent slot="activator">
...
computed: {
    userIsRegistered () {
        return this.$store.getters.user.registeredMeetups.findIndex(meetupId => {
            ...
            // si le résultat est négatif, je ne suis pas enregistré
        }) >= 0
    }
},
methods: {
